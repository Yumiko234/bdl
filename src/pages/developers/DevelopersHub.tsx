...
const DevelopersHub = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50 dark:bg-[#0b0f1a]">
      <Navigation />

      <main className="flex-1 relative overflow-hidden">
        {/* Fond : grille + halos */}
        <div className="dev-hub-grid absolute inset-0 pointer-events-none" />
        <div className="absolute -top-32 -left-32 h-96 w-96 rounded-full bg-blue-500/10 dark:bg-blue-500/20 blur-3xl pointer-events-none" />
        <div className="absolute -bottom-32 -right-32 h-96 w-96 rounded-full bg-amber-400/5 dark:bg-amber-400/10 blur-3xl pointer-events-none" />

        <section className="relative container mx-auto px-4 py-20 md:py-28">
          <div className="max-w-2xl mx-auto text-center space-y-4 mb-16">
            <div className="inline-flex items-center gap-2 text-xs font-mono uppercase tracking-widest text-blue-600/80 dark:text-blue-300/80 bg-blue-500/10 border border-blue-400/20 rounded-full px-4 py-1.5">
            ...
            <p className="font-mono text-sm md:text-base text-slate-500 dark:text-slate-400 h-6">
              {typed}
              <span className="animate-pulse">▍</span>
            </p>
            <p className="text-slate-600 dark:text-slate-400 max-w-xl mx-auto pt-2">
          ...
          <div className="grid sm:grid-cols-2 gap-6 max-w-3xl mx-auto">
            {developers.map((dev) => (
              ...
              >
                ...
                <div
                  className="relative h-24 w-24 rounded-full flex items-center justify-center text-2xl font-bold text-white ring-2 ring-black/10 dark:ring-white/10 group-hover:ring-[hsl(var(--dev-accent))] transition-all duration-300"
                ...
                <div className="relative space-y-1">
                  <h2 className="text-xl font-bold text-slate-900 dark:text-white">{dev.fullName}</h2>
                  {dev.handle && (
                    <p className="font-mono text-xs text-slate-500">{dev.handle}</p>
                  )}
                  <p className="text-sm text-slate-600 dark:text-slate-400">{dev.role}</p>
                </div>

                <div
                  className="relative inline-flex items-center gap-1.5 text-sm font-medium mt-2 px-4 py-2 rounded-full border border-black/10 dark:border-white/10 text-slate-800 dark:text-white/90 group-hover:text-[hsl(var(--dev-accent))] group-hover:border-[hsl(var(--dev-accent))]/40 transition-colors"
                >
                  Voir le profil
